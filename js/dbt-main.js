// dbt-main.js - Corrected and Reordered for Proper Hoisting/Scoping
...
let formProgress = 0; // Re-added, though not used for indicator, still useful for draft logic
let isDraftMode = false;

// --- 1. Utility Functions (Defined first for proper hoisting/scoping) ---
...
async function initializeApp() {
    try {
        ...
        // 2. Load Core Data (Schemes) - Now defined above
        await loadSchemes(); 
        ...
        // Ensure these are initialized AFTER all their dependencies (including global functions) are defined
        BudgetMaster.init(); 
        ReportManager.init(); 
...
setInterval(() => {
    if (formProgress > 10) { // Only save if some progress has been made
        const formData = collectFormData();
        localStorage.setItem('dbtFormDraft', JSON.stringify(formData));
    }
